...
Public Class Data_Supplier
    ...
    End Sub
    Sub tampilkan()
        DA = New OdbcDataAdapter("select * from table_supplier", CONN)
        DS = New DataSet
        DS.Clear()
        DA.Fill(DS)
    ...
    Sub autokode()
        Try
            Call koneksi()
            CMD = New OdbcCommand("Select * from table_supplier order by Kode_Supplier desc", CONN)
            ...
            Else
                ...
                If Len(txtkode.Text) = 1 Then
                    txtkode.Text = "S0" & txtkode.Text & ""
                ElseIf Len(txtkode.Text) = 2 Then
                    txtkode.Text = "S" & txtkode.Text & ""
                End If
            End If
    ...
    Sub MatikanForm()
        ...
        txttlpn.Enabled = False
    End Sub
    Sub HidupkanForm()
        txtkode.Enabled = False
        txtnama.Enabled = True
        txtalamat.Enabled = True
        txttlpn.Enabled = True
    End Sub
    ...
    Private Sub DGV_CellMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles DGV.CellMouseClick
        ...
        txttlpn.Text = DGV.Rows(e.RowIndex).Cells(3).Value
    ...
    Private Sub btsimpan_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btsimpan.Click
        If txtnama.Text = "" Or txtalamat.Text = "" Or txttlpn.Text = "" Then
            MsgBox("Data Belum Lengkap")
            Exit Sub
        Else
            Call autokode()
            Call koneksi()
            CMD = New OdbcCommand("select * from table_supplier where Kode_Supplier ='" & txtkode.Text & "'", CONN)
            DR = CMD.ExecuteReader
            DR.Read()
            If Not DR.HasRows Then
                Call koneksi()
                Dim Simpan As String
                Simpan = "insert into table_supplier values('" & txtkode.Text & "','" & txtnama.Text & "','" & txtalamat.Text & "','" & txttlpn.Text & "')"
                Dim Strpembanding As String = "Select * from table_supplier where Nama_Supplier = '" & txtnama.Text & "'"
    ...
    Private Sub btedit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btedit.Click
        ...
        Else
            Call tampilkan()
            Call koneksi()
            str = "update table_supplier set Nama_Supplier = '" & txtnama.Text & "' , Alamat = '" & txtalamat.Text & "', Telepon = '" & txttlpn.Text & "' where Kode_Supplier = '" & txtkode.Text & "'"
    ...
    Private Sub bthapus_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bthapus.Click
        ...
        Else
            If MessageBox.Show("Apakah Yakin Untuk Menghapusnya ?", "Konfirmasi", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
                Call koneksi()
                CMD = New OdbcCommand("delete from table_supplier where Kode_Supplier ='" & txtkode.Text & "'", CONN)
                CMD.ExecuteNonQuery()
    ...
    Private Sub txtpencarian_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtpencarian.TextChanged
        Call koneksi()
        CMD = New OdbcCommand("Select * From table_supplier Where Nama_Supplier Like '%" & txtpencarian.Text & "%'", CONN)
        DR = CMD.ExecuteReader
        DR.Read()
        If DR.HasRows Then
            Call koneksi()
            DA = New OdbcDataAdapter("Select * From table_supplier Where Nama_Supplier Like '%" & txtpencarian.Text & "%'", CONN)
            DS = New DataSet
            DA.Fill(DS)
            DGV.DataSource = DS.Tables(0)
        Else
